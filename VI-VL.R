...
srd$Popup1991<- left_join(srd@data,VLautomig %>% filter(Godina==1991)%>% group_by(Okrug) %>% summarise(popup=paste0(gsub(trimws(OpstinaNazivLat), pattern =  "Beograd-|Beograd -",replacement = "")," ", round(Doseljeno,1),"%", collapse = "\n")) , by=c("QNAME"="Okrug"))$popup
srd$Popup2002<- left_join(srd@data,VLautomig %>% filter(Godina==2002)%>%group_by(Okrug) %>% summarise(popup=paste0(gsub(trimws(OpstinaNazivLat), pattern =  "Beograd-|Beograd -",replacement = "")," ", round(Doseljeno,1),"%", collapse = "\n")) , by=c("QNAME"="Okrug"))$popup
srd$Popup2011<- left_join(srd@data,VLautomig %>% filter(Godina==2011)%>%group_by(Okrug) %>% summarise(popup=paste0(gsub(trimws(OpstinaNazivLat), pattern =  "Beograd-|Beograd -",replacement = "")," ", round(Doseljeno,1),"%" , collapse = "\n")) , by=c("QNAME"="Okrug"))$popup
